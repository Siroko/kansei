...
// Vertex data written as flat f32 array: 9 floats per vertex = 36 bytes
// (position vec4 + normal vec3 + uv vec2), matching the engine standard Vertex layout.
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
...

    {
        ...
        var nd = 0.0;
        if (!outside) { nd = sample_density(dims, vec3<u32>(ncoord)); }
        if (outside || nd < iso) { emit_quad(p100, p101, p111, p110, vec3<f32>(1.0, 0.0, 0.0), max_tris); }
    }
    {
        let ncoord = gid_i + vec3<i32>(-1, 0, 0);
        ...
        var nd = 0.0;
        if (!outside) { nd = sample_density(dims, vec3<u32>(ncoord)); }
        if (outside || nd < iso) { emit_quad(p001, p000, p010, p011, vec3<f32>(-1.0, 0.0, 0.0), max_tris); }
    }
    {
        ...
        var nd = 0.0;
        if (!outside) { nd = sample_density(dims, vec3<u32>(ncoord)); }
        if (outside || nd < iso) { emit_quad(p010, p110, p111, p011, vec3<f32>(0.0, 1.0, 0.0), max_tris); }
    }
    {
        let ncoord = gid_i + vec3<i32>(0, -1, 0);
        var outside = any(ncoord < vec3<i32>(0));
        var nd = 0.0;
        if (!outside) { nd = sample_density(dims, vec3<u32>(ncoord)); }
        if (outside || nd < iso) { emit_quad(p000, p001, p101, p100, vec3<f32>(0.0, -1.0, 0.0), max_tris); }
    }
    {
        ...
        var nd = 0.0;
        if (!outside) { nd = sample_density(dims, vec3<u32>(ncoord)); }
        if (outside || nd < iso) { emit_quad(p001, p011, p111, p101, vec3<f32>(0.0, 0.0, 1.0), max_tris); }
    }
    {
        let ncoord = gid_i + vec3<i32>(0, 0, -1);
        var outside = any(ncoord < vec3<i32>(0));
        var nd = 0.0;
        if (!outside) { nd = sample_density(dims, vec3<u32>(ncoord)); }
        if (outside || nd < iso) { emit_quad(p110, p010, p000, p100, vec3<f32>(0.0, 0.0, -1.0), max_tris); }
    }
}
`;
