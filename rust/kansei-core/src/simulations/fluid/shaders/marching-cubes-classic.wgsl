...
// moved out of the shader because Safari/WebKit's WGSL→Metal translator
...
// marching_cubes_tables.rs / marching-cubes-tables.ts). We bind them as
// uniform buffers instead of storage buffers so Chrome/Dawn routes the
// lookups through the GPU's constant cache (much faster for random-access
// tables like this). Each entry is packed 4-per-vec4 to match the WGSL
// uniform-array 16-byte stride requirement while keeping the upload as a
// flat byte copy of the u32/i32 arrays.
