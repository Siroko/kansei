...
pub async fn start(canvas_id: &str) -> Result<(), JsValue> {
    ...
    scene.add(SceneNode::Light(Light::Directional(dir_light)));
    ...
    scene.add(SceneNode::Light(Light::Point(point)));

    // Camera
    let mut camera = Camera::new(45.0, 0.1, 100.0, width as f32 / height as f32);
    camera.set_position(8.0, 6.0, 12.0);
    camera.look_at(&Vec3::new(0.0, 1.0, 0.0));
    camera.update_projection_matrix();

    log::info!("Kansei — Shadow Scene (WASM) ready");
